...
`mutsuki-0.1-alpha-3` 在 Runtime Wire 基线上继续纳入：可配置 QUIC Link TLS、Standalone Console
独立进程（`mutsuki-bot-console`），以及 `web_host` 作为 release-set 独立 repository（与
`bot_plugins` 解耦 pin）。ServiceHost / StdPlugins / BotPlugins revision 对齐到上述能力已推送的
远端 SHA；`capabilities.web_host = active`。
...
`MutsukiWebHost` **已列入** `releases/*.toml` 的独立 `[[repositories]]` 条目（`id = "web_host"`），
与 `bot_plugins` 解耦：release-set / Template 可单独 bump WebHost revision；`release_set.py sync`
会按该条目更新各仓库 Cargo Git pin，不必先发一版 BotPlugins 业务改动。

准入条件（均已满足）：

1. Standalone + Link 独立部署面：产品 binary `mutsuki-bot-console`（见
   [standalone-console.md](standalone-console.md)），进程级 smoke
   `standalone_console_process_smoke`；
2. 产品可配置的远程/QUIC TLS identity（`[link.quic]` + `web.console.quic_*`）与 loopback smoke；
3. WebHost 与 BotPlugins 解耦 pin：Template / release-set 显式 pin `mutsuki-web-host` /
   `mutsuki-web-protocol`；BotPlugins 对 WebHost 使用 Git `rev`（非仓内 path），由 release-set
   统一对齐。

验证路径：嵌入式 Console（`web_console_smoke`）、库装配 Standalone QUIC（`standalone_quic_smoke`）、
独立进程 Standalone（`standalone_console_process_smoke`）、BotPlugins console smoke。

升级 WebHost 时：更新 active manifest 的 `web_host.revision` → `sync` → Template
`cargo update` → `validate`；仅当 WebHost API 破坏 BotPlugins 扩展时才需同步改 BotPlugins 代码。
