...
`mutsuki-0.1-alpha-3` 精简产品面：仅保留同进程 Embedded Web Console（概览 / 插件 /
Runners / EventSources / 日志、配置页、升级 check/plan + Web dry-run、任务调试与生命周期强确认、
Secret 状态）。移除 Standalone Console 进程、`[link.quic]` 产品配置与 `quic://` 远程管理路径。
`web_host` 仍作为 release-set repository 条目以对齐 Cargo Git pin，但**不再**声明为
`capabilities.web_host = active` 独立产品能力。
...
`MutsukiWebHost` 列入 `releases/*.toml` 的 `[[repositories]]`（`id = "web_host"`）仅用于
**Cargo pin 追踪**：Embedded Console 依赖 WebHost 库；`release_set.py sync` 可单独 bump
`web_host.revision`。它不是可独立部署的产品能力，也不要求 `capabilities.web_host`。

验证路径：嵌入式 Console（`web_console_smoke`）与 BotPlugins console smoke。
